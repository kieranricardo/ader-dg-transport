...
class WaveAderDG2DImplicitAdjoint(BaseADERDG2D):
    ...
    def __init__(self, xlim, ylim, nx, ny, poly_order, c, dt, f=0.0, a=0.5):
        ...
        # dissipation terms
        # 0.5 * c * u * dy * dt
        self.M_nc[u_slice, u_slice] += self.a * self.x_cfl * (xm_integral + xp_integral)
        self.M_nc[v_slice, v_slice] += self.a * self.y_cfl * (ym_integral + yp_integral)
        self.M_nc[h_slice, h_slice] += self.a * self.x_cfl * (xm_integral + xp_integral)
        self.M_nc[h_slice, h_slice] += self.a * self.y_cfl * (ym_integral + yp_integral)
    ...
    def _xbdry_nc(self, bdry_integrals, arr, xp, xm):
        ...
        u_bdry_integrals[xp] += self.a * self.x_cfl * u_pred[xm] / self.weights_x[-1]

        u_bdry_integrals[xm] -= -0.5 * self.x_cfl * h_pred[xp] / self.weights_x[-1]
        u_bdry_integrals[xm] += self.a * self.x_cfl * u_pred[xp] / self.weights_x[-1]

        h_bdry_integrals[xp] += -0.5 * self.x_cfl * u_pred[xm] / self.weights_x[-1]
        h_bdry_integrals[xp] += self.a * self.x_cfl * h_pred[xm] / self.weights_x[-1]

        h_bdry_integrals[xm] -= -0.5 * self.x_cfl * u_pred[xp] / self.weights_x[-1]
        h_bdry_integrals[xm] += self.a * self.x_cfl * h_pred[xp] / self.weights_x[-1]
    ...
    def _ybdry_nc(self, bdry_integrals, arr, yp, ym):
        ...
        v_bdry_integrals[yp] += self.a * self.y_cfl * v_pred[ym] / self.weights_x[-1]

        v_bdry_integrals[ym] -= -0.5 * self.y_cfl * h_pred[yp] / self.weights_x[-1]
        v_bdry_integrals[ym] += self.a * self.y_cfl * v_pred[yp] / self.weights_x[-1]

        h_bdry_integrals[yp] += -0.5 * self.y_cfl * v_pred[ym] / self.weights_x[-1]
        h_bdry_integrals[yp] += self.a * self.y_cfl * h_pred[ym] / self.weights_x[-1]

        h_bdry_integrals[ym] -= -0.5 * self.y_cfl * v_pred[yp] / self.weights_x[-1]
        h_bdry_integrals[ym] += self.a * self.y_cfl * h_pred[yp] / self.weights_x[-1]
    ...
    def time_step(self, verbose=False, use_lu=True, rhs_in=None, maxiter=10, tol=0):
        ...
        for i in range(maxiter):
            state_pred_new = self.block_jacobi(state_pred, rhs_in)
