...
class WaveAderTrefftzConsDG2D(BaseADERDG2D):
    ...
    def __init__(self, xlim, ylim, nx, ny, poly_order, c, dt):
        ...
        # c
        self.M1 = ic_mm
        self.Mx = self.M1 + self.x_cfl * (xm_mm - xp_mm)
        self.My = self.M1 + self.y_cfl * (ym_mm - yp_mm)
        self.M = self.Mx + self.y_cfl * (ym_mm - yp_mm)
