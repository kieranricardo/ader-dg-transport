...
def get_interpolation_matrix(p_in, p_out):

    in_pts, _ = gll(p_in, iterative=True)
    out_pts, _ = gll(p_out, iterative=True)
    return get_interpolation_matrix_(in_pts, out_pts)


def get_interpolation_matrix_(in_pts, out_pts):

    vecs = []
    for i in range(len(in_pts)):
        y = np.zeros_like(in_pts)
        y[i] = 1
        poly = scipy.interpolate.lagrange(in_pts, y)
        vecs.append(poly(out_pts))

    interp_mat = np.stack(vecs, axis=1)
    return interp_mat


def get_interpolation_matrix_2D(px_in, py_in, px_out, py_out):

    x_in, _ = gll(px_in, iterative=True)
    y_in, _ = gll(py_in, iterative=True)
    x_out, _ = gll(px_out, iterative=True)
    y_out, _ = gll(py_out, iterative=True)

    interp_mat = np.zeros((py_out+1, px_out+1, py_in+1, px_in+1))
        ...
        for j in range(len(x_out)):
            for k in range(len(y_in)):
                for l in range(len(x_in)):
                    ...
                    interp_mat[i, j, k, l] = y_poly(y_out[i]) * x_poly(x_out[j])

    # interp_mat[i, j, k, l] =
    # for i, y_ in enumerate(y_in):
    #     for j, x_ in enumerate(x_in):
    #         y_data = np.zeros_like(y_in)
    #         y_data[i] = 1.0
    #         y_poly = scipy.interpolate.lagrange(y_in, y_data)
    #
    ...
    #         x_poly = scipy.interpolate.lagrange(x_in, x_data)
    #
    #         tmp = y_poly(y_out)[:, None] * x_poly(x_out)[None, :]
    #         interp_mat[:, :, i, j] = tmp
...
def get_upwind_dissipation(eig, poly_order):
    n = poly_order + 1

    polys = [scipy.special.legendre(i) for i in range(0, n)]
    xis, weights = gll(poly_order, iterative=True)

    norms = [(poly(xis) ** 2 * weights).sum() for poly in polys]
    polys = [poly / np.sqrt(norm) for poly, norm in zip(polys, norms)]
...
def get_upwind_D(eig, poly_order):
    S = get_upwind_dissipation(eig, poly_order)

    xis, weights = gll(poly_order, iterative=True)
    D = lagrange1st(poly_order, xis).transpose().copy()

    Dp = D + 0.5 * np.diag(1.0 / weights) @ S
    Dm = D - 0.5 * np.diag(1.0 / weights) @ S
