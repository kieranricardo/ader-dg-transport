...
def von_neumann_analysis(solver, cfl, x_shifts, y_shifts, batch_size=10_000):
    ...
    #only include x and y boundary terms
    Mx = np.copy(M1)
    My = np.copy(M1)
    
    Mx[u_slice, h_slice] += 0.5 * x_cfl * (xm_integral - xp_integral)
    # dvdt + dhdy = 0
    My[v_slice, h_slice] += 0.5 * y_cfl * (ym_integral - yp_integral)
    # dhdt + dudx + dvdy = 0
    Mx[h_slice, u_slice] += 0.5 * x_cfl * (xm_integral - xp_integral)
    My[h_slice, v_slice] += 0.5 * y_cfl * (ym_integral - yp_integral)

    # dissipation terms
    # 0.5 * c * u * dy * dt
    Mx[u_slice, u_slice] += 0.5 * x_cfl * (xm_integral + xp_integral)
    My[v_slice, v_slice] += 0.5 * y_cfl * (ym_integral + yp_integral)
    Mx[h_slice, h_slice] += 0.5 * x_cfl * (xm_integral + xp_integral)
    My[h_slice, h_slice] += 0.5 * y_cfl * (ym_integral + yp_integral)
